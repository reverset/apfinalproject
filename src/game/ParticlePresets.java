...
public class ParticlePresets {
    ...
    private static Function<ParticleEmitter.Particle, Color> fadeOut(Color color) {
        final Color desiredColor = color.cloneIfImmutable();
        return particle -> {
            double lifeRatio = particle.getTimeAlive() / particle.getLifetime();
            if (lifeRatio >= 0.66) {
    ...
    public static ParticleEmitter pop(int maxParticles, Color color) {
        return new ParticleEmitter(0, (float)MoreMath.TAU, maxParticles, 
            Duration.ZERO, Duration.ofMillis(400), Duration.ofMillis(400), 
            false, maxParticles, 10, SpawnStrategy.PERIMETER, 
            genericSquareRenderer(fadeOut(color)), 
            time -> 1.0, time -> 200.0);
    }

    public static ParticleEmitter flame(Color color) {
        return new ParticleEmitter((float)(MoreMath.TAU * 0.75), (float)(0.3f), 1000,
            Duration.ofMillis(10), Duration.ofSeconds(1), Duration.ZERO, 
            false, -1, 10, SpawnStrategy.AREA,
    ...
    public static ParticleEmitter dust() {
        return new ParticleEmitter(0, (float)MoreMath.TAU, 300, 
            Duration.ofMillis(100), Duration.ofSeconds(5), Duration.ZERO,
            false, -1, 1_000, SpawnStrategy.AREA,
